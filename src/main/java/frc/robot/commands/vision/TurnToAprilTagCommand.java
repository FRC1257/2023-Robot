...
public class TurnToAprilTagCommand extends CommandBase {
    ...
    double targetAngle; // Future variable, probably used in the case where the camera is not placed
                        // horizionally in front of the robot
    double kp; // scaling ratio for robot movement
    double error; // amount of error our robot detects that it tries to correct for, relative to
                  // the position of the AprilTag

    /**
     * Turns the robot using the gyro only for following an April Tag (tracked
     * target from VisionSubsystem)
     */
    ...
    public void execute() {
        if (vision.getHasTarget()) { // if we find a target,
            ...
            drivetrain.aprilTurn(-value, value); // write values to motors, negative and positive value in order for
                                                   // turning to occur
        } else {
            drivetrain.aprilTurn(0, 0); // otherwise, don't do anything
        }
    }
